...
public class DAOComment {
    ...
    public List<Comment_Blog> getListComment(int postId){
        try {
            ...
            String stmSql = "select a.Fullname,a.Profile_Picture,pc.Comment_Date,pc.Content from Account as a inner join Post_Comments as pc on a.Id = pc.Commenter_ID where pc.Post_ID = ?";
            Connection conn = new DBContext().getConnection();
            PreparedStatement ps = conn.prepareStatement(stmSql);
            ps.setInt(1, postId);
            ResultSet rs = ps.executeQuery();
            while(rs.next()){
                Comment_Blog cb = new Comment_Blog(rs.getString(1), rs.getString(2), rs.getDate(3), rs.getString(4));
                lsC.add(cb);
            }
            return lsC;
        } catch (Exception e) {
